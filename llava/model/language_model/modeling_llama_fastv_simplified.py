...
class FineFastVLlamaModelSimplified(LlamaModel):
    """
    简化版的动态Head选择模型
    核心思想：基于实际效果的简单策略，去除复杂的多阶段处理
    """
    ...
    def __init__(self, config: LlamaConfig, visual_token_num: int):
        ...
        print(f"🔧 Simplified head selection initialized with strategy: {self.head_selection_strategy}")
    ...
    def forward(
        ...
        # Head Selection
        if hasattr(self.config, 'H') and self.config.H is not None:
            self.H = self.config.H
            print(f"Head number updated to: {self.H}")
                ...
                if seq_length > 1:
                    ...
                    if (decoder_layer.self_attn.layer_idx + 1) == self.K:
                        ...
                        attn_mask = _prepare_4d_causal_attention_mask(
                            attn_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length
                        )
                        ...
                        # 使用策略选择heads并聚合attention
                        visual_head_index, aggregated_attention = self.select_heads_by_strategy(image_attention)

                        # 选择top-R visual tokens
                        visual_token_index = torch.topk(aggregated_attention, k=self.R).indices
                        visual_token_index = torch.sort(visual_token_index).values
